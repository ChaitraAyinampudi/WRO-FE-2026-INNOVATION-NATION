...
def build_vehicle_driving(model_path: str, drive_mode: str,
                          show_camera: bool = False) -> dk.vehicle.Vehicle:
    car = dk.vehicle.Vehicle()
    add_camera(car)
    ...
    car.add(DynamixelSteering(), inputs=["drive/angle"])
    car.add(DynamixelThrottle(), inputs=["drive/throttle"])
    return car
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Record data, drive normally, or drive with a live camera window"
    )
    ...
    group.add_argument(
        ...
        help="Autonomous model driving with a live camera window"
    )
    parser.add_argument(
        "--model", default=None,
        help="Optional .h5 model path for either driving mode"
    )
    parser.add_argument(
    ...
    if args.recording:
        ...
        show_camera = bool(args.driving_view)
        print("Driving with camera view selected." if show_camera
              else "Normal driving selected.")
        ...
        vehicle = build_vehicle_driving(
            str(mpath), drive_mode=drive_mode, show_camera=show_camera
        )
