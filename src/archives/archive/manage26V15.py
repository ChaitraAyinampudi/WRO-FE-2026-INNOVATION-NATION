...
GYRO_THREE_LAP_TARGET_DEG = 1440.0
...
GYRO_MAX_INTEGRATION_DT_SECONDS = 0.25
...
class GyroThreeLapController:
    ...
    def run(self, angle, throttle):
        ...
        if self.last_sample_time is None:
            ...
            if dt < 0.0:
                dt = 0.0
            elif dt > GYRO_MAX_INTEGRATION_DT_SECONDS:

                dt = GYRO_MAX_INTEGRATION_DT_SECONDS

            if raw_z is not None and math.isfinite(raw_z):
                corrected_rad_per_sec = raw_z - self.gyro_bias_rad_per_sec
                rate_deg_per_sec = math.degrees(corrected_rad_per_sec)
                ...
                if abs(rate_deg_per_sec) <= GYRO_MAX_VALID_RATE_DEG_PER_SEC:
                    self.total_rotation_deg += rate_deg_per_sec * dt
                else:
                    print(
        ...
        progress = abs(self.total_rotation_deg)

        if now - self.last_print_time >= 1.0:
            current_rate = 0.0
            if raw_z is not None and math.isfinite(raw_z):
                current_rate = math.degrees(raw_z - self.gyro_bias_rad_per_sec)
                if abs(current_rate) < GYRO_RATE_DEADBAND_DEG_PER_SEC:
                    current_rate = 0.0
        ...
        if self.finish_deadline is None and progress >= GYRO_THREE_LAP_TARGET_DEG:
            ...
            print(
                f"Gyro target reached at {progress:.1f} degrees. "
        ...
        if self.finish_deadline is not None and now >= self.finish_deadline:
            self.stopped = True
            print(
                "Gyro-target finish delay complete. Steering centered and throttle stopped.",
                flush=True,
            )
            return 0.0, 0.0
