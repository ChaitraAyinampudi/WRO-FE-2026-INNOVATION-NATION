...
GYRO_THREE_LAP_TARGET_DEG = 1080.0
FREE_RUN_FINISH_SECONDS = 3.0
OBSTACLE_RUN_FINISH_SECONDS = 3.5

GYRO_RAW_AXIS = "z"
GYRO_CALIBRATION_SECONDS = 2.0
GYRO_CALIBRATION_SAMPLE_SECONDS = 0.01
GYRO_RAW_DEADBAND_RAD_S = 0.02
GYRO_MAX_DT_SECONDS = 0.25
...
def obstacle_start_program(drive_mode: str):
    ...
    try:
        if mode == "OCW":
            ...
            run_position(DXL_STEER_ID, 100, 36, bus=bus)
            time.sleep(0.15)
            run_for_degrees(DXL_THROTTLE_IDS, 16, 86, bus=bus)

            run_position(DXL_STEER_ID, 100, -29, bus=bus)
            time.sleep(0.15)
            run_for_degrees(DXL_THROTTLE_IDS, 16, 65, bus=bus)

            run_position(DXL_STEER_ID, 100, 0, bus=bus)
            time.sleep(0.15)
            run_for_degrees(DXL_THROTTLE_IDS, 16, 19, bus=bus)

        else:

            run_position(DXL_STEER_ID, 100, 36, bus=bus)
            time.sleep(0.15)
            run_for_degrees(DXL_THROTTLE_IDS, 16, 69, bus=bus)

            run_position(DXL_STEER_ID, 100, -29, bus=bus)
            time.sleep(0.15)
            run_for_degrees(DXL_THROTTLE_IDS, 16, 50, bus=bus)

            run_position(DXL_STEER_ID, 100, 0, bus=bus)
            time.sleep(0.15)
            run_for_degrees(DXL_THROTTLE_IDS, 16, 10, bus=bus)
...
class GyroThreeLapController:
    ...
    def __init__(self, drive_mode: str):
        ...
        self.gyro_bias = self._calibrate_raw_gyro()
        ...
        print(
            f"Sense HAT raw {GYRO_RAW_AXIS.upper()} gyro armed for {run_name} run: "
            f"stop after {GYRO_THREE_LAP_TARGET_DEG:.0f} degrees plus "
            f"{self.finish_seconds:.1f} seconds.",
            flush=True,
        )

    def _read_raw_rate(self):
        try:
            raw = self.sense.get_gyroscope_raw()
            return float(raw[GYRO_RAW_AXIS])
        except Exception as exc:
            print(f"Sense HAT raw gyro read failed: {exc}", flush=True)
            return None

    def _calibrate_raw_gyro(self) -> float:
        print(
            f"Calibrating raw {GYRO_RAW_AXIS.upper()} gyro for "
            f"{GYRO_CALIBRATION_SECONDS:.1f} seconds. Keep the car still...",
            flush=True,
        )
        ...
        while time.monotonic() < deadline:
            rate = self._read_raw_rate()
            if rate is not None:
                samples.append(rate)
            time.sleep(GYRO_CALIBRATION_SAMPLE_SECONDS)

        if not samples:
            print("Raw gyro calibration failed; using zero bias.", flush=True)
            return 0.0

        bias = sum(samples) / len(samples)
        print(
            f"Raw gyro calibrated from {len(samples)} samples: "
            f"bias={bias:+.6f} rad/s.",
            flush=True,
        )
        return bias
    ...
    def run(self, angle, throttle):
        ...
        if self.last_sample_time is None:
            ...
            if 0.0 < dt <= GYRO_MAX_DT_SECONDS and raw_rate is not None:
                corrected_rate = raw_rate - self.gyro_bias

                if abs(corrected_rate) < GYRO_RAW_DEADBAND_RAD_S:
                    corrected_rate = 0.0

                delta_degrees = math.degrees(corrected_rate * dt)
                self.total_degrees += delta_degrees

        progress = abs(self.total_degrees)

        if now - self.last_print_time >= 1.0:
            rate_deg_s = 0.0
            if raw_rate is not None:
                rate_deg_s = math.degrees(raw_rate - self.gyro_bias)
            print(
                f"Raw gyro: {rate_deg_s:+.1f} deg/s | "
                f"lap progress: {progress:.1f} / "
                f"{GYRO_THREE_LAP_TARGET_DEG:.0f} degrees",
                flush=True,
            )
            self.last_print_time = now

        if self.finish_deadline is None and progress >= GYRO_THREE_LAP_TARGET_DEG:
            self.finish_deadline = now + self.finish_seconds
            print(
                f"Three laps detected at {progress:.1f} degrees. "
        ...
        if self.finish_deadline is not None and now >= self.finish_deadline:
            self.stopped = True
            print(
                "Three-lap finish delay complete. Steering centered and throttle stopped.",
                flush=True,
            )
            return 0.0, 0.0
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description=(
            ...
            "Sense HAT raw-gyro three-lap stopping"
        )
    )
