...
class GyroThreeLapController:
    ...
    def __init__(self, drive_mode: str):
        ...
        print(
            f"Sense HAT gyro armed for {run_name} run: "
    ...
    @staticmethod
    def _wrapped_delta(current_yaw: float, previous_yaw: float) -> float:
        return (current_yaw - previous_yaw + 180.0) % 360.0 - 180.0

    def _read_yaw(self):
        try:
            orientation = self.sense.get_gyroscope()
            return float(orientation["yaw"])
        except Exception as exc:
            print(f"Sense HAT gyro read failed: {exc}", flush=True)
            return None
    ...
    def run(self, angle, throttle):
        ...
        if yaw is not None:
            if self.last_yaw is None:
                self.last_yaw = yaw
                print(f"Gyro starting yaw: {yaw:.1f} degrees", flush=True)
            else:
                delta = self._wrapped_delta(yaw, self.last_yaw)
                self.last_yaw = yaw

                if abs(delta) <= GYRO_MAX_VALID_STEP_DEG:
                    self.total_yaw += delta
                else:
                    print(
                        f"Ignored unusual gyro jump of {delta:+.1f} degrees.",
                        flush=True,
                    )

        progress = abs(self.total_yaw)

        if now - self.last_print_time >= 1.0:
            print(
                f"Gyro lap progress: {progress:.1f} / "
